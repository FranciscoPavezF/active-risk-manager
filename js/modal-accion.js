...
async function generarSiguienteIdAccion() {
    ...
    try {
        ...
        while (idsNumericos.includes(siguiente)) {
            siguiente++;
        }

        return `AC-${String(siguiente).padStart(3, "0")}`;

    } catch (error) {

        console.error(
            "ERROR AL GENERAR ID DE ACCIÓN:",
            error
        );
    ...
    botonNuevaAccion.addEventListener(
        ...
        async function () {
            ...
            campoId.value =
    await generarSiguienteIdAccion();
    ...
    botonGuardar.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                // OBJETO NORMALIZADO
                // =============================================
const usuarioActivoCreacionAccion =
    window.VireonSesion &&
    ...
    calcularPrioridadAutomatica(
        ...
        registroAsociado
                    ...
                    responsable:
                        ...
                        "Sin asignar",
creadoPor:
    usuarioActivoCreacionAccion?.nombre ||
    "Sistema VIREON",
                    fechaCompromiso:
                        fechaCompromiso,
...
if (accionAnterior?.creadoPor) {
    datosAccion.creadoPor =
        accionAnterior.creadoPor;
}
const usuarioActivoEdicionAccion =
    window.VireonSesion &&
    typeof window.VireonSesion.obtenerUsuarioActual === "function"
        ? window.VireonSesion.obtenerUsuarioActual()
        : null;
...
if (accionAnterior.estado !== datosAccion.estado) {
    cambiosAccion.push(
        `Estado: ${accionAnterior.estado} → ${datosAccion.estado}`
    );
}

if (Number(accionAnterior.avance || 0) !== Number(datosAccion.avance || 0)) {
    cambiosAccion.push(
        `Avance: ${Number(accionAnterior.avance || 0)}% → ${Number(datosAccion.avance || 0)}%`
    );
}

if ((accionAnterior.responsable || "Sin asignar") !== (datosAccion.responsable || "Sin asignar")) {
    cambiosAccion.push(
        `Responsable: ${accionAnterior.responsable || "Sin asignar"} → ${datosAccion.responsable || "Sin asignar"}`
    );
}

if (accionAnterior.fechaCompromiso !== datosAccion.fechaCompromiso) {
    cambiosAccion.push(
        `Fecha compromiso: ${accionAnterior.fechaCompromiso || "-"} → ${datosAccion.fechaCompromiso || "-"}`
    );
}

const claveTrazabilidadAccion =
    `vireon_trazabilidad_${datosAccion.id}`;
...
try {
    historialAccion =
        JSON.parse(
            localStorage.getItem(claveTrazabilidadAccion)
        ) || [];
} catch (error) {
    historialAccion = [];
}

historialAccion.push({
    tipo: "actualizacion",
    titulo: "Acción actualizada",
    descripcion:
        cambiosAccion.length
            ? cambiosAccion.join(" · ")
            : "Se guardó la acción sin cambios relevantes.",
    responsable:
        usuarioActivoEdicionAccion?.nombre ||
        "Sistema VIREON",
    fecha: new Date().toISOString()
});

localStorage.setItem(
    claveTrazabilidadAccion,
    JSON.stringify(historialAccion)
);
    ...
    .agregar(
        datosAccion
                    ...
                    const claveTrazabilidadCreacionAccion =
    `vireon_trazabilidad_${datosAccion.id}`;

const historialCreacionAccion = [
    {
        tipo: "creacion",
        titulo: "Acción creada",
        descripcion: "La acción fue registrada.",
        responsable:
            datosAccion.creadoPor ||
            "Sistema VIREON",
        fecha: new Date().toISOString()
...
localStorage.setItem(
    claveTrazabilidadCreacionAccion,
    JSON.stringify(historialCreacionAccion)
);
