...
const grupoExposicionEconomica =
    document.getElementById(
        "grupoExposicionEconomica"
    );

const campoExposicionEconomica =
    document.getElementById(
        "nuevaExposicionEconomica"
    );
    ...
    function configurarFormularioPorTipo() {
        ...
        if (
            ...
            if (grupoExposicionEconomica) {
    grupoExposicionEconomica.classList.add("hidden");
}
...
if (grupoExposicionEconomica) {
    grupoExposicionEconomica.classList.remove("hidden");
}
            if (labelPlan) {
    ...
    function limpiarFormulario() {
        ...
        campoPlan.value =
            ...
            if (campoExposicionEconomica) {
    campoExposicionEconomica.value = "";
}
  ...
  const usuarioActivoCreacionRiesgo =
    window.VireonSesion &&
...
exposicionEconomica:
    Number(campoExposicionEconomica?.value || 0),
...
creadoPor:
    usuarioActivoCreacionRiesgo?.nombre ||
    "Sistema VIREON",
...
datosRiesgo.creadoPor =
    riesgoAnterior?.creadoPor ||
    datosRiesgo.creadoPor;

datosRiesgo.exposicionEconomica =
    Number(campoExposicionEconomica?.value || 0);
...
if (
    Number(riesgoAnterior.exposicionEconomica || 0) !==
    Number(datosRiesgo.exposicionEconomica || 0)
) {
    cambios.push(
        `Exposición económica: US$ ${Number(
            riesgoAnterior.exposicionEconomica || 0
        ).toLocaleString("es-CL")} → US$ ${Number(
            datosRiesgo.exposicionEconomica || 0
...
const usuarioActivoEventoRiesgo =
    window.VireonSesion &&
    typeof window.VireonSesion.obtenerUsuarioActual === "function"
        ? window.VireonSesion.obtenerUsuarioActual()
        : null;
...
responsable:
    usuarioActivoEventoRiesgo?.nombre ||
    "Sistema VIREON",
...
const usuarioActivoCreacionOportunidad =
    window.VireonSesion &&
    typeof window.VireonSesion.obtenerUsuarioActual === "function"
        ? window.VireonSesion.obtenerUsuarioActual()
        : null;
...
creadoPor:
    usuarioActivoCreacionOportunidad?.nombre ||
    "Sistema VIREON",
...
if (
    Number(oportunidadAnterior.beneficioPotencial || 0) !==
    Number(datosOportunidad.beneficioPotencial || 0)
) {
    cambios.push(
        `Beneficio potencial: US$ ${Number(
            oportunidadAnterior.beneficioPotencial || 0
        ).toLocaleString("es-CL")} → US$ ${Number(
            datosOportunidad.beneficioPotencial || 0
        ).toLocaleString("es-CL")}`
    );
}
...
const usuarioActivoEventoOportunidad =
    window.VireonSesion &&
    typeof window.VireonSesion.obtenerUsuarioActual === "function"
        ? window.VireonSesion.obtenerUsuarioActual()
        : null;
...
historial.push({
   ...
   responsable:
    usuarioActivoEventoOportunidad?.nombre ||
    "Sistema VIREON",
